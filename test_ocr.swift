...
func parseCalories(from lines: [String]) -> Int? {
    for (index, line) in lines.enumerated() {
        ...
        if line.range(of: "\\bcalor[io0]e?s?\\b", options: [.regularExpression, .caseInsensitive]) != nil {
            print("  → Found 'Calories' word on line")
                ...
                for match in matches {
                    if let numberRange = Range(match.range, in: line),
                       ...
                       number >= 50 && number < 10000 {
                        print("  → Found calories on same line: \(number)")
                        return number
                    }
                }
            }

            print("  → No valid calorie number on 'Calories' line, checking adjacent lines")

            // If no number on same line, check previous line
            if index > 0 {
                let previousLine = lines[index - 1]
                print("  → Checking previous line: '\(previousLine)'")
                ...
                if let regex = try? NSRegularExpression(pattern: #"\d+"#, options: []) {
                    ...
                    if let largest = calorieNumbers.max() {
                        print("  → Found calories from previous line: \(largest)")
                        return largest
                    }
                }
            }
            ...
            if index < lines.count - 1 {
                ...
                if let regex = try? NSRegularExpression(pattern: #"\d+"#, options: []) {
                    ...
                    let calorieNumbers = numbers.filter { $0 >= 50 && $0 < 10000 }
                    if let largest = calorieNumbers.max() {
                        print("  → Found calories from next line: \(largest)")
                        return largest
                    }
                }
            }
